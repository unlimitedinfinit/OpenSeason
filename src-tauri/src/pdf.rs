...
#set page(
  ...
  footer: align(center)[
    #text(fill: red, size: 8pt, weight: "bold")[
      THIS IS A USER-GENERATED CONFIDENTIAL DISCLOSURE STATEMENT CREATED BY OPEN SEASON. IT IS NOT LEGAL ADVICE.
    ]
  ]
)
...
#align(center)[
  ...
  #v(0.2em)
  #text(size: 10pt, weight: "bold", fill: red)[CONFIDENTIAL -- FOR ATTORNEY WORK PRODUCT / DOJ REVIEW ONLY]
]

#v(1em)

*TO:* United States Department of Justice\
*FROM:* Relator (Anonymous Whistleblower)\
*DATE:* {date}
...
= I. Summary
Relator submits this disclosure statement pursuant to 31 U.S.C. 3730(b)(2). The evidence enclosed herein demonstrates that the Target entity has knowingly presented false or fraudulent claims for payment to the United States Government.

- **Target Entity Name:** {target}
- **Estimated Contract Exposure Value:** \${value}

= II. Fraud Timeline & Chronology
The following events trace the billing anomalies, false certifications, and internal directives supporting the fraud allegations:

{timeline}

= III. Evidence Locker Log
The evidence below has been securely archived, audited, and encrypted locally within the relator's vault:

#table(
  columns: (1.2fr, 1.8fr, 2.5fr),
  inset: 6pt,
  fill: (_, y) => if y == 0 {{ gray.lighten(80%) }} else {{ none }},
  align: horizon,
  [*File Name*], [*Description of Proof*], [*SHA-256 Hash Checksum*],
  {evidence}
)

= IV. Sealed Complaint Narrative Draft
The preliminary factual counts and allegations compiled for the Qui Tam filing are outlined below:

{complaint}

#v(2fr)
#align(right)[
  #text(size: 8pt, style: "italic", fill: gray)[Generated securely by Open Season Zero-Trust Toolkit]
]
"#,
        target = target_name,
        date = chrono::Local::now().format("%Y-%m-%d"),
        value = total_value,
        timeline = if timeline_markup.is_empty() { "_No events logged in timeline._" } else { timeline_markup },
        evidence = if evidence_markup.is_empty() { "  [No evidence files], [No evidence logged], [N/A]\n" } else { evidence_markup },
