...
| Evidence encrypt + EXIF strip | **Works** | Hunt uploads strip JPEG/PNG metadata, hash, and encrypt with XChaCha20Poly1305. Nonce is stored in SQLite for those uploads. Adding the same bytes twice is a no-op and does not overwrite the content-hash `.enc` (important after seal). Desktop seal walks the whole tree (nested evidence and root-level user files), prefixes the nonce on the ciphertext, decrypt-verifies, then deletes plaintext. Drag-and-drop in the UI still opens a file picker. `metadata.db` itself is still plaintext. |
| Timeline / parties / complaint sections | **Partial** | Standard cases store parties and timeline on `case.json`. Confidential vaults still have SQLite tables for hunt-era data. The UI is one party editor and one dashboard. Opening an existing vault hunt writes a Documents `case.json` pointer so it appears as a Confidential case. Vault exhibit names are listed on that case. |
| USAspending + disclosure | **Partial** | Real HTTP POST to `api.usaspending.gov` from Confidential tools on the unified case dashboard. The leftover HuntWizard screen was removed. Disclosure uses the exhibit count and a $0 placeholder the user must edit. The live USAspending test prints results and does not assert. Network is optional intel, not case upload. |
...
| Tests | **Partial** | Case-mode, seal, evidence (including sealed add/delete), unlock, path sandbox, Notice of Appeal, and basic complaint/motion assembly tests are in `cargo test`. The live USAspending test is `#[ignore]` because it hits the network. There are no end-to-end UI tests. |
