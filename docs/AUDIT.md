...
| `.osb` import/export | **Works** | Zip write/read with zip-slip protection. Import creates a new UUID folder under `vaults/` so later hunt commands stay inside the sandbox. Export refuses app data, the cases root, a file named `.sealed`, and overwriting an existing file. |
