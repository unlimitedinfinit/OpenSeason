...
{:else if currentState === "DASHBOARD"}
  <main class="container mx-auto p-8 space-y-8">
    <div class="flex justify-between items-center border-b pb-4">
      <div>
        <div>
          <button onclick={() => goto('/')} class="text-xs text-muted-foreground hover:text-foreground">Back to home</button>
          <h1 class="text-3xl font-bold tracking-tight">Confidential mode</h1>
          <p class="text-muted-foreground">Open Season vault. These cases never sync.</p>
        </div>
      </div>
       ...
       <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <GlassOSPanel borderRadius={8} scale={15} depth={20} enableSpecular={true}>
          <button 
            ...
            class="w-full h-full p-6 flex flex-col items-center justify-center space-y-4 border border-dashed border-primary/50 hover:bg-accent/10 transition-colors relative"
            disabled={isCreating}
          >
              {#if isCreating}
        ...
        <GlassOSPanel borderRadius={8} scale={15} depth={20} enableSpecular={true}>
          <button 
             onclick={startDemo}
             class="w-full h-full p-6 flex flex-col items-center justify-center space-y-3 transition-all group"
             disabled={isCreating}
          >
        ...
        {#each hunts as hunt}
            <GlassOSPanel borderRadius={8} scale={12} depth={15} enableSpecular={true}>
              <div 
                class="p-6 cursor-pointer group relative"
                onclick={() => goto(`/hunt/${hunt.id}`)}
              >
